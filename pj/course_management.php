...
function _remember_new_opt($id){
  if (!isset($_SESSION['hot_new_option_ids'])) $_SESSION['hot_new_option_ids'] = [];
  $id = (int)$id;
  if ($id > 0 && !in_array($id, $_SESSION['hot_new_option_ids'], true)) {
    $_SESSION['hot_new_option_ids'][] = $id;
  }
}
/* <<< END */
...
try { $pdo->exec("ALTER TABLE form_options ADD COLUMN created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP"); } catch (Throwable $e) { /* already */ }
...
if (isset($_GET['export'])) {
  ...
  if ($what === 'courses') {
    fputcsv($out, ['รหัสวิชา','ชื่อวิชา','หน่วยกิต','คณะ','สาขา','สาขาวิชา','หลักสูตร','ปีหลักสูตร','ปีที่แนะนำ','วิชาบังคับ(1/0)','วิชาบังคับก่อน']);
    if (!$headerOnly) {
  ...
  if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action_course'])) {
    ...
    if ($act === 'edit') {
      ...
      if ($code === '' || $name === '' || $course_id<=0) {
        flash('danger','กรอก “รหัสวิชา/ชื่อวิชา” ให้ครบ');
      } else {
        try {
          $sql = "UPDATE courses
                  SET course_code=?,
                      ...
                      major_value=?,
                      program_value=?,
                      curriculum_name_value=?,
                      curriculum_year_value=?,
                      is_compulsory=?,
                      recommended_year=?,
                      prereq_text=?
                  WHERE course_id=?";
          ...
          if (substr_count($sql,'?') !== count($params)) {
            throw new Exception('PLACEHOLDER_MISMATCH: '.substr_count($sql,'?').' vs '.count($params));
          }
    ...
    if ($act === 'delete') {
      ...
      if ($course_id > 0) {
        try {
          $st = $pdo->prepare("DELETE FROM courses WHERE course_id=?");
          $st->execute([$course_id]);
          flash('ok', 'ลบรายวิชาเรียบร้อย');
        } catch (Throwable $e) {
          flash('danger', 'เกิดข้อผิดพลาดในการลบ: ' . $e->getMessage());
        }
      } else {
        flash('danger', 'ไม่พบรหัสรายวิชาที่ต้องการลบ');
      }
    }
...
if ($view === 'options') {
  if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action_opt'])) {
    ...
    if ($act === 'import_opts_csv') {
      ...
      try {
        ...
        while (($row = fgetcsv($fh)) !== false) {
          ...
          $success++;
          _remember_new_opt((int)$pdo->lastInsertId());
        }
        fclose($fh);
        $pdo->commit();
    ...
    if ($act==='del_opt') {
      ...
      if ($id>0){
        try {
          $st = $pdo->prepare("DELETE FROM form_options WHERE id=?");
          $st->execute([$id]);
          flash('ok','ลบตัวเลือกเรียบร้อย');
        } catch (Throwable $e) { flash('danger','ลบไม่สำเร็จ: '.$e->getMessage()); }
      } else {
        flash('danger','ไม่พบ ID ของตัวเลือกที่จะลบ');
      }
    }
...
<div class="container">
    ...
    <div class="card" style="margin-bottom:24px; display:grid; gap:16px;">
      ...
      <form method="post" enctype="multipart/form-data" style="display:grid; grid-template-columns: 1fr; gap:12px;">
        ...
        <div class="form-field">
          <label for="csv">
            นำเข้า CSV (course_code,course_name,credits,
            <span style="color:#9ca3af">[optional] recommended_year,is_compulsory,prereq_text</span>)
          </label>
      ...
      <form method="post">
        ...
        <div class="form-field">
            ...
            <p style="font-size:12px; color:var(--muted); margin-top:8px;">
              รูปแบบ: <strong>รหัสวิชา,ชื่อวิชา,หน่วยกิต[,ปีที่แนะนำ(1-5),วิชาบังคับ(1/0),วิชาบังคับก่อน]</strong> (ไม่ใส่หน่วยกิต = 3.0)
            </p>
        </div>
        <div class="form-field" style="margin-bottom:0;">
    ...
    <div class="card"><div class="table-wrap"><table class="table">
      <thead>
        <tr>
          ...
          <th style="width:120px">วิชาบังคับ</th>
        ...
        <tr>
          ...
          <td>
            <?php echo !empty($c['is_compulsory']) 
              ? '<span class="badge" style="background:rgba(225,29,72,.15);color:var(--rose)">บังคับ</span>' 
  ...
  <?php else: ?>
                        ...
                        <td style="font-weight:600">
                          ...
                          <?php
                            ...
                            if (isset($o['created_at'])) {
                              ...
                              if ($ts && (time() - $ts) <= 24*3600) $isRecent = true;
                            }
                            if ($isHot || $isRecent): ?>
    ...
    <div class="card">
      ...
      <div class="card" style="margin-bottom:24px; display:grid; gap:16px;">
        <div style="display:flex; gap:12px; flex-wrap:wrap; align-items:center;">
          <a class="btn secondary" href="<?php echo e($_SERVER['PHP_SELF']); ?>?view=options&export=options&header_only=1">⬇️ ส่งออก CSV (เฉพาะหัวตาราง)</a>
    ...
    <form method="post" id="courseEditForm">
      ...
      <div class="form-field">
        ...
        <label style="display:flex;align-items:center;gap:8px;">
          ...
          <span>ติ๊กถ้าเป็นวิชาบังคับ</span>
        </label>
      </div>
      <div class="form-field">
        <label>ปีการศึกษาที่แนะนำ (1–5)</label>
        <input class="input" type="number" min="1" max="5" name="recommended_year" id="course_edit_recyr" placeholder="เช่น 2">
      </div>
      <div class="form-field">
        <label>วิชาบังคับก่อน (พิมพ์ข้อความ เช่น ผ่าน CS101)</label>
