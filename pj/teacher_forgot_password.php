<?php
// teacher_forgot_password.php — ขอรีเซ็ตรหัสผ่านโดยยืนยันด้วย username + teacher_code
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $teacher_code === '') {
        ...
        // หาอาจารย์ด้วย "username + teacher_code"
        $sql = "SELECT teacher_id, username, teacher_code
                FROM teacher
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            $error = 'ไม่สามารถค้นหาข้อมูลได้: ' . $conn->error;
        }
        ...
        if (!$error) {
            if (!$teacher) {
                ...
                $teacherId = (string)$teacher['teacher_id'];  // ใช้ teacher_id เป็น identifier
                $expires   = date('Y-m-d H:i:s', time() + 60*60*12); // 12 ชม.
                $token     = ''; // โหมดรอแอดมินอนุมัติ ยังไม่ต้องมี token

                // UPSERT โดยมี unique key (user_type, identifier)
                $sqlUpsert = "
