<?php
// groups_manage.php — จัดการกลุ่มเรียน (ผูก education_info.student_group & curriculum_name เป็นตัวหนังสือ)
session_start();
...
/* ===== Migrate ที่ต้องใช้ใน course_groups ===== */
try{
...
function eiStatusExpr(PDO $pdo){
  foreach (['status','enrollment_status','student_status'] as $c)
    if (colExists($pdo,'education_info',$c)) return "ei.`{$c}`";
  return "'enrolled'";
}
function eiEnrollDateExpr(PDO $pdo){
  foreach (['created_at','enrolled_at','updated_at','reg_date'] as $c)
    if (colExists($pdo,'education_info',$c)) return "ei.`{$c}`";
  return "NULL";
}
...
/* ===== Query blocks (ใช้ t.id แทน t.teacher_id) ===== */
function groupsByCurriculum(PDO $pdo, $cur, $is_admin, $is_teacher, $uid){
  $base = "
    ...
    LEFT JOIN teacher t ON cg.teacher_id = t.id
    WHERE cg.curriculum_value = ?
  ";
  if ($is_admin){
    $st=$pdo->prepare("$base ORDER BY cg.group_name"); $st->execute([$cur]);
  } else {
    $st=$pdo->prepare("$base AND cg.teacher_id=? ORDER BY cg.group_name"); $st->execute([$cur,$uid]);
  }
  return $st->fetchAll(PDO::FETCH_ASSOC);
}
...
if (isset($_GET['ajax'])){
  ...
  if ($ajax==='groups_table'){
    ...
    if ($cur===''){ echo '<div class="muted">โปรดเลือกหลักสูตร</div>'; exit; }
  ...
  if ($ajax==='roster'){
    ...
    if ($gid<=0){ echo '<div class="muted">ไม่พบกลุ่ม</div>'; exit; }
    $info = groupInfo($pdo,$gid,$user_id,$is_admin,$is_teacher);
    if (!$info){ echo '<div class="muted">ไม่พบกลุ่มหรือสิทธิ์ไม่พอ</div>'; exit; }
...
/* ===== POST actions (เพิ่ม/แก้/ลบ/เพิ่ม-นำออก นศ.) ===== */
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action'])){
  ...
  if ($act==='add_group' || $act==='edit_group'){
    ...
    if($teacher_id==='') $errs[]='โปรดเลือกอาจารย์ที่ปรึกษา';
    if($max_students<1 || $max_students>500) $errs[]='จำนวนนักศึกษาสูงสุด 1–500';
    $ckT=$pdo->prepare("SELECT 1 FROM teacher WHERE id=? LIMIT 1"); // ใช้ id
    $ckT->execute([$teacher_id]);
    ...
    if ($act==='add_group'){
      ...
      if ($max_students < $curCount){
        flash('danger','จำนวนนักศึกษาปัจจุบันมากกว่าค่าสูงสุดใหม่'); header("Location: ".$_SERVER['PHP_SELF']); exit;
      }
...
<style>
  /* =========================
    THEME TOKENS (Dark/Glass)
    ========================= */
  :root{
    --navy:#0f172a; --steel:#1e293b; --sky:#3b82f6; --sky-500:#0ea5e9; --sun:#fde68a;
  ...
  *{box-sizing:border-box}
  ...
  body{
    margin:0;font-family:'Sarabun',system-ui,Segoe UI,Roboto,sans-serif;color:var(--text);
    background:
  ...
  .container{max-width:1280px;margin:0 auto;padding:20px 14px}
  ...
  .sky{
    background:linear-gradient(135deg,var(--sky-500),#57caf5);
    color:#fff;
    border-color:transparent
  }
  .btn-sm{
  ...
  /* Toolbar ที่ยืดหยุ่นและไม่ซ้อนกัน */
  .toolbar{
    display:flex;
    gap:8px;
    align-items:center;
  ...
  /* สไตล์ select + dropdown ให้เข้ากับธีม */
  select.select{
    appearance:none;
    background-image:
...
<div class="container">
  <div class="topbar">
    <div class="brand">
      ...
      <div><h1>จัดการกลุ่มเรียน</h1><div class="subtitle">เพิ่ม แก้ไข หรือลบกลุ่มเรียน</div></div>
    </div>
    <div class="top-actions">
      <?php if ($is_admin): ?><a href="admin_dashboard.php" class="btn secondary"><i class="fas fa-home"></i>หน้าหลัก</a><?php endif; ?>
    </div>
  </div>
    ...
    <div class="toolbar">
      <select id="curriculumSelect" class="select" onchange="reloadGroups()">
        ...
        <?php foreach($curricula as $cur): ?>
          <option value="<?php echo e($cur['value']); ?>" <?php echo ($cur['value']===$selected_cur)?'selected':''; ?>><?php echo e($cur['label']); ?></option>
        <?php endforeach; ?>
      </select>
      <select id="groupSelect" class="select" onchange="if(this.value){ loadRoster(this.value); }">
        <option value="">— เลือกกลุ่ม —</option>
        <?php foreach($groups_top as $g): ?>
          <option value="<?php echo (int)$g['group_id']; ?>"><?php echo e($g['group_name']); ?></option>
        <?php endforeach; ?>
      </select>
      <?php if ($is_admin): ?><button class="btn primary" onclick="openAddGroup()"><i class="fas fa-plus"></i> เพิ่มกลุ่มเรียน</button><?php endif; ?>
    </div>
  </div>

  <?php foreach ($flashes as $f): ?>
    <div class="alert <?php echo $f['t']==='ok'?'ok':'dangerA'; ?>"><i class="fas <?php echo $f['t']==='ok'?'fa-circle-check':'fa-triangle-exclamation'; ?>"></i><span><?php echo e($f['m']); ?></span></div>
  <?php endforeach; ?>

  <div class="columns">
    <div class="card"><h3 style="margin:0 0 8px"><i class="fas fa-list-ul"></i> กลุ่มเรียนในหลักสูตร</h3><div id="groupsArea"><div class="muted">โปรดเลือกหลักสูตร</div></div></div>
    <div class="card"><h3 style="margin:0 0 8px"><i class="fas fa-users"></i> รายชื่อนักศึกษาในกลุ่ม</h3><div id="rosterArea"><div class="muted">เลือกกลุ่มจากดรอปดาวน์ หรือกด “รายชื่อ” จากตาราง</div></div></div>
  </div>
</div>

<?php if ($is_admin): ?>
<!-- Modals: Add/Edit/Delete/Enroll (เหมือนเดิม) -->
<div id="groupModal" class="modal"><div class="modal-content"><div class="modal-header"><h3 id="gmTitle">เพิ่มกลุ่มเรียน</h3><span class="close" onclick="closeModal('groupModal')">&times;</span></div>
<form method="post" id="groupForm">
<input type="hidden" name="csrf_token" value="<?php echo e($_SESSION['csrf_token']); ?>">
<input type="hidden" name="action" value="add_group" id="gmAction">
<input type="hidden" name="group_id" id="gm_group_id">
<div class="form-grid">
  <div class="form-field"><label>หลักสูตร</label><select name="curriculum_value" id="gm_curriculum" required>
    <option value="">— เลือกหลักสูตร —</option><?php foreach($curricula as $cur): ?><option value="<?php echo e($cur['value']); ?>"><?php echo e($cur['label']); ?></option><?php endforeach; ?>
  </select></div>
  <div class="form-field"><label>กลุ่มเรียน</label><select name="group_name" id="gm_group_name" required>
    <option value="">— เลือกกลุ่ม —</option><?php foreach($groupNames as $gn): ?><option value="<?php echo e($gn['label']); ?>"><?php echo e($gn['label']); ?></option><?php endforeach; ?>
  </select></div>
  <div class="form-field"><label>อาจารย์ที่ปรึกษา</label><select name="teacher_id" id="gm_teacher_id" required>
    <option value="">— เลือกอาจารย์ —</option><?php foreach($teachers as $t): ?><option value="<?php echo e($t['teacher_id']); ?>"><?php echo e($t['teacher_name']); ?></option><?php endforeach; ?>
  </select></div>
  <div class="form-field"><label>จำนวนนศ.สูงสุด</label><input type="number" min="1" max="500" name="max_students" id="gm_max" value="40" required></div>
</div>
<div style="display:flex;gap:10px;justify-content:flex-end;margin-top:10px">
<button type="button" class="btn cancel" onclick="closeModal('groupModal')">ยกเลิก</button>
<button type="submit" class="btn primary">บันทึก</button>
</div>
</form></div></div>

<div id="delGroupModal" class="modal"><div class="modal-content"><div class="modal-header"><h3>ยืนยันการลบกลุ่ม</h3><span class="close" onclick="closeModal('delGroupModal')">&times;</span></div>
<form method="post">
<input type="hidden" name="csrf_token" value="<?php echo e($_SESSION['csrf_token']); ?>">
...
<div style="display:flex;gap:10px;justify-content:flex-end;margin-top:10px">
<button type="button" class="btn cancel" onclick="closeModal('delGroupModal')">ยกเลิก</button>
<button type="submit" class="btn danger">ลบ</button>
</div>
</form></div></div>

<div id="enrollModal" class="modal"><div class="modal-content">
...
<div style="display:flex;gap:10px;justify-content:flex-end;margin-top:10px">
<button type="button" class="btn cancel" onclick="closeModal('enrollModal')">ยกเลิก</button>
...
function loadGroupsTableForCurrentCur(){
  ...
  fetch(`?ajax=groups_table&curriculum_value=${encodeURIComponent(cur)}`).then(r=>r.text()).then(html=>{ $('groupsArea').innerHTML=html; });
}
<?php if ($is_admin): ?>
...
document.addEventListener('DOMContentLoaded', function () { loadGroupsTableForCurrentCur(); });
</script>
</body>
</html>
