...
/**
 * register.php — ฟอร์มลงทะเบียนนักศึกษา (พร้อมวาง)
 * - ดึงตัวเลือก dropdown ผ่าน JSON endpoint ภายในไฟล์นี้เอง
 * - เก็บค่าจาก dropdown เป็น "label" ทั้งหมด (ตามตาราง form_options)
 * - หลังบันทึกสำเร็จ แสดงข้อความบนหน้าเดิม + เด้งไปหน้า login.php ภายใน 3 วินาที
 */

require __DIR__ . '/db_connect.php'; // ต้องประกาศ $conn = new mysqli(...)
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  try {
    ...
    // ตรวจความถูกต้องขั้นพื้นฐาน
    if ($full_name === '' || $student_id === '' || $password === '') {
      throw new Exception("กรุณากรอก: ชื่อ-นามสกุล, รหัสนักศึกษา, รหัสผ่าน");
    }
    if (!ctype_digit($student_id)) throw new Exception("รหัสนักศึกษาต้องเป็นตัวเลขเท่านั้น");
    if ($phone !== '') {
    ...
    // รหัสนักศึกษาห้ามซ้ำใน education_info
    ...
    // 2) education_info (เก็บ label ตามสคีมาปัจจุบัน)
    ...
    // 3) user_login (ถ้ามีตารางนี้อยู่)
    if (table_exists($conn, 'user_login')) {
      // กันซ้ำ username ถ้ามีอยู่ก่อน
      $chk = $conn->prepare("SELECT 1 FROM user_login WHERE username=? LIMIT 1");
      $chk->bind_param("s", $student_id);
      $chk->execute();
      $dupLogin = $chk->get_result()->num_rows > 0;
      if ($dupLogin) {
        // ถ้ามีอยู่แล้ว ข้าม (ถือว่าเคยสร้างแล้ว)
      } else {
    ...
    // *** สำคัญ: ไม่ล็อกอินอัตโนมัติ และไม่ redirect ไป dashboard ***
    // ล็อกอินอัตโนมัติ แล้วเด้งไปแดชบอร์ด
  ...
  <div class="form-container">
    ...
    <form method="POST" action="" id="regForm" novalidate>
      ...
      <div class="form-group">
        ...
        <input type="text" id="citizen_id" name="citizen_id" class="form-control" maxlength="13" inputmode="numeric" autocomplete="off" placeholder="เลข 13 หลัก ไม่ต้องมีขีด">
      </div>
      <div class="form-group">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" id="phone" name="phone" class="form-control" inputmode="numeric" maxlength="10" autocomplete="off" placeholder="เช่น 0812345678">
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input type="text" id="student_id" name="student_id" class="form-control" required inputmode="numeric" maxlength="15" autocomplete="off" placeholder="กรอกรหัสนักศึกษา (ตัวเลขเท่านั้น)">
        </div>
      </div>
      <div class="form-row">
...
<?php if (!empty($success_message)): ?>
<!-- เด้งไปหน้าเข้าสู่ระบบอัตโนมัติใน 3 วินาที (ตัดบล็อกนี้ทิ้งได้ ถ้าไม่อยากให้เด้ง) -->
